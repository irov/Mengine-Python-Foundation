class RPGFormules(object):
    ...
    def calcFormula(self, name, Value=None, Global=None, **Other):
        ...
        def __rand(value): return Mengine.randf(value)
        def __fibo(value): return Mengine.fibo_bine(value)
        def __log10(value): return Mengine.log10f(value)
        def __resist(value): return (1.0 - (0.06 * value) / (1.0 + 0.06 * value)) if value >= 0.0 else 1.0 / (1.0 - pow(0.94, -value))
        def __evade(value): return 1.0 if Mengine.randf(100.0) <= value else 0.0
        def __sum(value): return sum(value)
